...
resource "azurerm_linux_function_app" "helloworld_function" {
  name                                           = var.helloworld_function_service_name
  resource_group_name                            = var.resource_group_name
  location                                       = var.location
  ...
  ############################################################################
  ### https://github.com/hashicorp/terraform-provider-azurerm/issues/25775 ###
  ############################################################################
  # zip_deploy_file = data.archive_file.helloworld.output_path
}
...
resource "azurerm_linux_function_app" "assistant_function" {
  ...
  app_settings = {
    ...
    STORAGE_CONTAINER_NAME = azurerm_storage_container.videos_in.name
    # storage_account_name           = data.azurerm_storage_account.functions_storage.name
    ...
    videoindexer_account_id = var.videoindexer_account_id
  }
  site_config {
    application_stack {
      python_version = "3.11"
...
#   }
#   app_settings = {
#     # AzureWebJobsStorage__accountName = data.azurerm_storage_account.functions_storage.name
#     APPINSIGHTS_INSTRUMENTATIONKEY = var.instrumentation_key
#     ENABLE_ORYX_BUILD              = true
...
#   # zip_deploy_file =  "https://github.com/Azure/ai-hub/raw/main/infrastructure/terraform/modules/functions/rag.zip"
# }