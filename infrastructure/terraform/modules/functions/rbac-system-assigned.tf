# Assiging RBAC to underlying Storage Account for the function apps - helloworld
resource "azurerm_role_assignment" "storage_account_blob_owner_helloworld" {
  scope                = data.azurerm_storage_account.functions_storage.id
  role_definition_name = "Storage Blob Data Owner"
  principal_id         = azurerm_linux_function_app.helloworld_function.identity[0].principal_id
}
resource "azurerm_role_assignment" "storage_account_queue_contributor_helloworld" {
  scope                = data.azurerm_storage_account.functions_storage.id
  role_definition_name = "Storage Queue Data Contributor"
  principal_id         = azurerm_linux_function_app.helloworld_function.identity[0].principal_id
}

resource "azurerm_role_assignment" "storage_account_table_contributor_helloworld" {
  scope                = data.azurerm_storage_account.functions_storage.id
...
resource "azurerm_role_assignment" "video_storage_account_owner_helloworld" {
...
resource "azurerm_role_assignment" "aoai_cognitive_service_openai_user_helloworld" {
