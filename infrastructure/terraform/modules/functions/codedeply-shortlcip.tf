...
resource "azurerm_resource_group_template_deployment" "shortclip_function_code_deploy_nested_deployment" {
  name                = "code-deployment-${azurerm_storage_blob.shortclip_function_blob.name}"
...
TEMPLATE
  ...
  parameters_content = jsonencode({
    "appServiceName" : {
      "value" : "${azurerm_linux_function_app.shortclip_function.name}"
    },
    "packageUri" : {
      "value" : ("${azurerm_storage_blob.shortclip_function_blob.url}${data.azurerm_storage_account_sas.assistant_function_blob_sas.sas}")
    }
  })
}