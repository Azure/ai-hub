...
TEMPLATE
  ...
  parameters_content = jsonencode({
    "appServiceName" : {
      "value" : "${azurerm_linux_function_app.assistant_function.name}"
    },
    "packageUri" : {
      "value" : ("${azurerm_storage_blob.assistant_function_blob.url}${data.azurerm_storage_account_sas.assistant_function_blob_sas.sas}")
    }
  })
}